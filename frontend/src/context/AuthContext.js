...
export const AuthProvider = ({ children }) => {
        ...
        const loadUser = async () => {
            ...
            if (storedToken) {
                try {
                    ...
                    const res = await axios.get('https://kam-udaan-4.onrender.com/api/auth/me');
    ...
    const login = async (email, password) => {
        try {
            const res = await axios.post('https://kam-udaan-4.onrender.com/api/auth/login', { email, password });
    ...
    const register = async (name, email, password, timezone = 'UTC') => {
        try {
            const res = await axios.post('https://kam-udaan-4.onrender.com/api/auth/register', { name, email, password, timezone });
            const { token: receivedToken, user } = res.data;
            localStorage.setItem('token', receivedToken);
            axios.defaults.headers.common['Authorization'] = `Bearer ${receivedToken}`;
            setUser(user);
